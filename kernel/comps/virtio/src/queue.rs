...
impl<const SIZE: usize> VirtQueue<SIZE> {
    ...
    pub fn add_buf(&mut self, inputs: &[&[u8]], outputs: &[&mut [u8]]) -> Result<u16, QueueError> {
        // FIXME: use `DmaStream` for inputs and outputs. Now because the upper device driver lacks the
    ...
    /// Adds DmaReader/DmaWriter to the virtqueue, return a token.
    ///
    /// Ref: linux virtio_ring.c virtqueue_add
    pub fn add_dma(
        &mut self,
        ...
        let desc_head = self.desc_table.add_dma(inputs, outputs);
        self.aval_ring.push_desc(desc_head);
        Ok(desc_head)
    }
...
impl DescTable {
    ...
    pub fn add_dma(&mut self, inputs: &[DmaReader], outputs: &[DmaWriter]) -> u16 {
