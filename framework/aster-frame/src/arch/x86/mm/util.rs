...
pub unsafe fn fast_copy<T>(src: *const T, dst: *mut T, count: usize) {
    ...
    // There is overlap and `src` is lower than `dst`.
    if src < dst && src.add(count) > dst {
        ...
        // Since there is overlap, perform the copy in reverse by byte.
        core::arch::asm!(
            "std", // Set the direction flag (DF) to copy in reverse
            "rep movsb",
            in("rcx") bytes_count,
            in("rsi") last_src,
            in("rdi") last_dst,
    ...
    // There is overlap and `src` is higher than `dst`.
    else if src > dst && src < dst.add(count) {
        let bytes_count = count * core::mem::size_of::<T>();

        // Since there is overlap, perform the copy by byte.
        core::arch::asm!(
            "rep movsb",
            in("rcx") bytes_count,
            in("rsi") src,
    ...
    // There is no overlap.
        ...
        fast_copy_nonoverlapping(src, dst, count);
    }
}
